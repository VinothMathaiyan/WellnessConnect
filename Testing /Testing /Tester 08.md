# WellnessConnect — Tester 8 Guide (Lyra)
...
> the names that start with **Lyra** (your names). Leave everyone else's alone.
...
|------|------------------------|------------------------|----------------------|
| **Client** | `Lyra Client` | `9100000157` | `123456` |
| **Trainer** | `Lyra Trainer` | `9200000157` | `123456` |
...
**Do this:**
1. Open the website in incognito. Sign up with phone **`9100000157`**, code `123456`.
2. When asked to choose a role, pick **Client**. Enter the name **`Lyra Client`**.
3. Fill in the short profile it asks for (age, goals, health details, etc.) and submit.
4. Open a **new incognito window**. Sign up with phone **`9200000157`**, code `123456`.
5. Choose **Trainer**. Enter the name **`Lyra Trainer`**. Complete the trainer details
...
2. In the list, find **`Lyra Trainer`** (your trainer — ignore all other names).
3. Open it, review the details, and tap **Approve**.
4. Now switch to your **Trainer** account (`9200000157`).
...
2. Find **`Lyra Client`** (ignore other names).
3. Open it. You should be able to **see the client's health details** (age, goals,
   ...
   **Clear for Training**.
4. Now switch to your **Client** account (`9100000157`). If it still shows the pending
...
**Do this:**
1. As **Client** (`9100000157`) → **Trainers** → find and open **`Lyra Trainer`**
...
2. On the trainer's profile, tap **Request Call Back**. (This sends a connection request to
   that trainer.)
3. Switch to your **Trainer** (`9200000157`) → go to **My Clients**.
4. Under **Requests**, you'll see **`Lyra Client`** marked **PENDING** → tap it to open the
   request.
...
- Tap **Accept Client** → **`Lyra Client`** moves into the trainer's **active clients**.
...
**Do this:**
1. As **Trainer** → open **`Lyra Client`** → scroll down to the **Current Program** section →
   tap **Create Program** → build/assign a simple program → save it.
2. Switch to your **Client** account (`9100000157`) → open **Alerts** → tap the
...
**Do this:**
1. As **Trainer** → open **`Lyra Client`** → **Schedule Session**.
...
5. Switch to **Client** (`9100000157`) → look at **Home / upcoming sessions** → open the session.
...
**Do this:**
1. As **Trainer** → open **`Lyra Client`** → find the session you scheduled **today** →
   tap **Complete**.
...
**Do this:**
1. As **Trainer** (`9200000157`) → open the **Risk** area.
2. Find **`Lyra Client`** (from the bad-day check-in in Test 8) → open the alert →
...
- After escalating: as Assessment Team, the escalation for **`Lyra Client`** appears in the
...
**A) Client → Trainer**
1. As **Client** (`9100000157`) → **Messages** → tap **New**.
2. In the picker you should see your trainer (**Lyra Trainer**) under "My Trainer(s)" and
   **Assessment Team** as another option. Pick **Lyra Trainer**.
3. Send a message like "Hello from Lyra Client".
...
5. Switch to **Trainer** (`9200000157`) → **Messages**. You should see the message from
   **Lyra Client**. Open it and **reply** ("Got it, thanks").
6. The trainer can also start a new one: **New** → pick **Lyra Client** or **Assessment
   Team** → send a message.

**D) Assessment Team → anyone (with search)**
7. Switch to **Assessment Team** (`9600000001`) → **Messages** → **New**. There's a
   **search box** — type "Lyra" → you should see **Lyra Trainer** and **Lyra Client** in the
...
**Do this:**
1. As **Trainer** → open **`Lyra Client`** → **Schedule Session** and create **a few sessions
...
3. On the **Trainer's** view of **`Lyra Client`**, look at the **Adherence score**.
