# WellnessConnect — Tester 8 Guide
...
> the names that start with **T8** (your tester number). Leave everyone else's alone.
...
|------|------------------------|------------------------|----------------------|
| **Client** | `T8 Client` | `9100000108` | `123456` |
| **Trainer** | `T8 Trainer` | `9200000108` | `123456` |
...
**Do this:**
1. Open the website in incognito. Sign up with phone **`9100000108`**, code `123456`.
2. When asked to choose a role, pick **Client**. Enter the name **`T8 Client`**.
3. Fill in the short profile it asks for (age, goals, etc.) and submit.
4. Open a **new incognito window**. Sign up with phone **`9200000108`**, code `123456`.
5. Choose **Trainer**. Enter the name **`T8 Trainer`**. Complete the trainer details
...
2. In the list, find **`T8 Trainer`** (your trainer — ignore all other names).
3. Open it, review the details, and tap **Approve**.
4. Now switch to your **Trainer** account (`9200000108`).
...
2. Find **`T8 Client`** (ignore other names).
3. Open it, fill in the short assessment, set a fitness level, and choose **Clear for Training**.
4. Now switch to your **Client** account (`9100000108`). If it still shows the pending
...
**Do this:**
1. As **Client** (`9100000108`) → **Trainers** → open any trainer's profile → send a
   **request to connect** (or "request callback").
2. Switch to your **Trainer** (`9200000108`) → go to **My Clients**.
3. Find the pending request from **`T8 Client`** → tap **Accept**.

**You should see:**
- As Trainer, **`T8 Client`** moves into your **active clients** list after you accept.
...
**Do this:**
1. As **Trainer** → open **`T8 Client`** → **Program Builder** → create/assign a simple
   program → save.
...
**Do this:**
1. As **Trainer** → open **`T8 Client`** → **Schedule Session**.
...
**Do this:**
1. As **Trainer** → open **`T8 Client`** → find the session you scheduled **today** →
   tap **Complete**.
...
**Do this:**
1. As **Trainer** (`9200000108`) → open the **Risk** area.
2. Find **`T8 Client`** (from the bad-day check-in in Test 8) → open the alert →
...
- After escalating: as Assessment Team, the escalation for **`T8 Client`** appears in the
...
**Do this:**
1. As **Trainer** → open **`T8 Client`** → send a message like "Hello from your trainer".
...
5. *(Assessment Team channel)* As **Assessment Team** (`9600000001`) → **Messages** → send a
   message to **`T8 Trainer`**. Then switch to your **Trainer** account and open Messages.
...
**Do this:**
1. As **Trainer** → open **`T8 Client`** → **Schedule Session** and create **a few sessions
