...
**Do this:**
1. As **Trainer** → open **`Tone Client`** → scroll down to the **Current Program** section →
   tap **Create Program** → build/assign a simple program → save it.
2. Switch to your **Client** account (`9100000101`) → open **Alerts** → tap the
...
5. Switch to **Client** (`9100000101`) → look at **Home / upcoming sessions** → open the session.
...
**A) Client → Trainer**
1. As **Client** (`9100000101`) → **Messages** → tap **New**.
2. In the picker you should see your trainer (**Tone Trainer**) under "My Trainer(s)" and
   **Assessment Team** as another option. Pick **Tone Trainer**.
3. Send a message like "Hello from Tone Client".
...
5. Switch to **Trainer** (`9200000101`) → **Messages**. You should see the message from
   **Tone Client**. Open it and **reply** ("Got it, thanks").
6. The trainer can also start a new one: **New** → pick **Tone Client** or **Assessment
   Team** → send a message.

**D) Assessment Team → anyone (with search)**
7. Switch to **Assessment Team** (`9600000001`) → **Messages** → **New**. There's a
   **search box** — type "Tone" → you should see **Tone Trainer** and **Tone Client** in the
...
3. On the **Trainer's** view of **`Tone Client`**, look at the **Adherence score**.
